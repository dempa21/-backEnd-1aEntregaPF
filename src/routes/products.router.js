...
router.get("/:pid", async function(req, res) {
  ...
  router.post("/", async (req, res) => {
    ...
    req.body.id ? res.send({ error: "El id es autogenerado" }) :
    product.title ? product.title : res.send({ error: "El título del producto es obligatorio" });
    product.description ? product.description : res.send({ error: "La descripción del producto es obligatoria" });
    product.code ? product.code : res.send({ error: "El código del producto es obligatorio" });
    product.price ? product.price : res.send({ error: "El precio del producto es obligatorio" });
    product.status ? product.status : res.send({ error: "El status del producto es obligatorio" });
    product.stock ? product.stock : res.send({ error: "El stock del producto es obligatorio" });
    product.category ? product.category : res.send({ error: "La categoría del producto es obligatoria" });
    // si no hay productos genera un id = 1, sino es autoincrementable
    productos.some(e => e.id === 1) ? product.id = Math.max(...productos.map(o => o.id)) + 1 : product.id = 1;
    if (productos.some(e => e.id === product.id)) { return res.send({ error: "Producto con id duplicado" });}
